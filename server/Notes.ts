...
export class Notes {
  ...
  private readonly move_attachments_note: (
    i: number
  ) => Statement<(number | bigint)[]>;
  private readonly find_missing_tags: (
    i: number
  ) => Statement<(number | bigint)[]>;
  ...
  constructor(db: Database, att: Attachments, tagService: Tags) {
    this.add_note_to_inbox = db.prepare(
      "insert into Notes \
    ...
    this.add_note = db.prepare(
      "insert into Notes \
      (notebookId, createTime, updateTime, title, noteData, updatedBy) values \
      ($notebookId, $createTime, date('now'), $title, $noteData, $updateBy)"
    );

    this.get_note_data = prepare_many(
      db,
      "select title, noteData, createTime, notebookId, GROUP_CONCAT(a.hash) hashes, GROUP_CONCAT(t.name) tags\
        ...
        where n.noteId in (#noteIds) group by n.noteId",
      "#noteIds"
    );

    this.select_body = db
      .prepare("select noteData from Notes where noteId = ?")
      .raw();

    this.update_note_data = db.prepare(
      "update Notes set noteData = ? where noteId = ?"
    );

    this.move_attachments_note = prepare_many(
      db,
      "update Attachments set noteId = ? where noteId in (#noteIds)",
      "#noteIds"
    );

    this.find_missing_tags = prepare_many(
      db,
    ...
    this.delete_notes = prepare_many(
      db,
      "delete from Notes where noteId in (#noteIds)",
      "#noteIds"
    );

    this.count_parts = db
      .prepare(
        "select length(replace(noteData, 'paperless-merged-note-data', 'paperless-merged-note-data1')) - length(noteData) as parts from Notes where noteId = ?"
      )
      .raw(true);
  ...
  mergeNotes(
    ...
    // add missing tags
    this.find_missing_tags(notes.length)
      .all(...notes, toNote)
      .forEach((tagId) => this.tagService.addTagId(toNote, tagId.tagId));
    //delete orphan tags
  ...
  verifyNotNullOrWhitespace(
    input: string | null | undefined,
    callback: (x: string) => unknown
  ) {
    const x = input?.trim();
    if (x) {
      callback(x);
            ...
            part.childNodes.forEach((dataNode) => {
              ...
              switch (data.className) {
                ...
                case "paperless-merged-note-attachments":
                  this.verifyNotNullOrWhitespace(
                    ...
                    (hashString) => {
                      ...
                      if (hashes) {
                        attachments.push(
                          ...hashes.map((h: string) => {
                            return { hash: h, noteId: originalNoteId };
                          })
                        );
                      }
                    }
                ...
                case "paperless-merged-note-create-date":
                  newNote.createTime = data.textContent ?? "0";
                  break;
              }
            });
            break;
