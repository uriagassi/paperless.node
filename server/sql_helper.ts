...
function prepare_one(
  ...
  return db.prepare(
    template.replace(trigger, [...Array(number)].map(() => "?").join(","))
  );
}

export function prepare_many(
  db: Database,
  template: string,
  trigger: string,
  number = 10
  ...
  return (i: number) =>
    get((i: number) => prepare_one(db, template, trigger, i), result, i);
}

function get(
  preparer: (i: number) => Statement,
  prepared: Statement[],
  amount: number
): Statement {
