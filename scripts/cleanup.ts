...
const delete_unused_tags = db.prepare(
  "delete from Tags where tagId not in (select parentId from Tags where parentId is not null) and tagId not in (select tagId from NoteTags)"
);

const find_unused_tags = db.prepare(
  "select name from Tags where tagId not in (select parentId from tags where parentId is not null) and tagId not in (select tagId from notetags)"
);
...
function findOrphanFiles() {
  const connectedFiles = new Set(
    find_all_filenames.all().map(({ uniqueFileName }) => uniqueFileName)
  );
  const files = fs.readdirSync(attachmentsDir).filter((f: string) => {
    return (
      !f.startsWith(".") && fs.lstatSync(path.join(attachmentsDir, f)).isFile()
    );
  });
    ...
    switch (s) {
      case "1": {
        const orphanPath = path.join(
          config.get("paperless.baseDir"),
          "orphans"
        );
        fs.mkdirSync(orphanPath);
        orphanFiles.forEach((f: string) => {
          fs.renameSync(path.join(attachmentsDir, f), path.join(orphanPath, f));
        });
        break;
      }
      case "2":
