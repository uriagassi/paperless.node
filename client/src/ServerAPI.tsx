...
export class ServerAPI {
  ...
  loadNotebooks(): Promise<{ tags: ITagWithChildren[]; notebooks: ITagWithChildren[] }> {
    return this.make_call("/api/notebooks_and_tags").then(
      (res) => res.json() as Promise<{ tags: ITagWithChildren[]; notebooks: ITagWithChildren[] }>
    );
  }

  loadNotes(filter: string, limit?: number): Promise<{ notes: Note[] }> {
    return this.make_call("/api/" + filter + "limit=" + (limit ?? 100) + "&lastItem=0").then((res) => res.json());
  }

  loadNote(noteId: string | number): Promise<RawNote> {
    return this.make_call("/api/notes/" + noteId).then((res) => res.json() as Promise<RawNote>);
  }
